...
const CAP_DEFAULT_SEVERITY = env.getString(
  'CAP_DEFAULT_SEVERITY',
  'Unknown'
);
const CAP_DEFAULT_CERTAINTY = env.getString(
  'CAP_DEFAULT_CERTAINTY',
  'Unknown'
);
const CAP_DEFAULT_AREADESC = env.getString(
...
const normalizeAlertDates = (alert) => {
  ...
  // normalize sent date
  normalizedAlert.sent = common.isValue(normalizedAlert.sent)
    ? new Date(normalizedAlert.sent)
    : undefined;

  // normalize effective date
  normalizedAlert.info.effective = common.isValue(normalizedAlert.info.effective)
    ? new Date(normalizedAlert.info.effective)
    : normalizedAlert.sent;

  // normalize onset date
  normalizedAlert.info.onset = common.isValue(normalizedAlert.info.onset)
    ? new Date(normalizedAlert.info.onset)
    : normalizedAlert.info.effective;

  // normalize expires date
  normalizedAlert.info.expires = common.isValue(normalizedAlert.info.expires)
...
const isValidAlert = (alert, optns) => {
  ...
  if (partial) {
    const isPartialAlert =
      common.isValue(copyOfAlert.info) &&
      common.isValue(copyOfAlert.info.event) &&
      common.isValue(copyOfAlert.info.effective) &&
      common.isValue(copyOfAlert.info.expires) &&
      common.isValue(copyOfAlert.info.instruction) &&
      common.isValue(copyOfAlert.info.area) &&
      common.isValue(copyOfAlert.info.area.areaDesc);
  ...
  // same as specs
  const isFullAlert =
    common.isValue(copyOfAlert.identifier) &&
    common.isValue(copyOfAlert.sender) &&
    common.isValue(copyOfAlert.sent) &&
    common.isValue(copyOfAlert.status) &&
    common.isValue(copyOfAlert.msgType) &&
    common.isValue(copyOfAlert.scope) &&
    common.isValue(copyOfAlert.info) &&
    common.isValue(copyOfAlert.info.category) &&
    common.isValue(copyOfAlert.info.event) &&
    common.isValue(copyOfAlert.info.urgency) &&
    common.isValue(copyOfAlert.info.severity) &&
    common.isValue(copyOfAlert.info.certainty) &&
    common.isValue(copyOfAlert.info.effective) &&
    common.isValue(copyOfAlert.info.expires) &&
    common.isValue(copyOfAlert.info.instruction) &&
    common.isValue(copyOfAlert.info.area) &&
...
exports.isValidAlert = isValidAlert;
